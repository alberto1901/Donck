...
void water_geometry_func(out vec4 oPosition, out vec3 oNormal)
{
    ...
    //calculate wave height of crest
    float h = amplitude * cos(angle);

    //calculate wave steepness by pulling x closer to the crest
		float f = k * (oPosition.x - wave_number_rad_ft * wave_time_sec);
		float x = amplitude * sin(f);

    // Reduce amplitude and level towards the edges.
    float d = clamp(1.0 - 0.005 * (length(oPosition.xy) - 50.0), 0.01, 1.0);

    // z = vertical; x = front-to-back; y = side-to-side
    oPosition.z += h * sqrt(d); //works
    oPosition.x += x * sqrt(d);
//    oPosition.y += h*sqrt(d); // not needed?
